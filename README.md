...
security role from the client, but it does detect one. On every entries
...
revisited before going to production. Seeing this warning during a UAT
session with two or more test accounts is the practical way to confirm
isolation is (or isn't) actually enforced, since it can't be verified from
the app code alone.
