 ...
 *
 * The Power SDK is loaded lazily inside the host-only branch, and that
 * `import()` is load-bearing for the *test* graph, not the bundle (#116).
 * `@microsoft/power-apps/data` has an extensionless internal import that
 * Node's ESM resolver cannot follow, so a static import here would break
 * every unit test that renders the calendar — the calendar imports this
 * module, which is why the deferral has to stay.
 *
 * What it does NOT buy is a smaller initial download: `src/generated/`
 * imports the same two modules statically, so they are reachable from the
 * entry either way and the build used to say so out loud
 * (INEFFECTIVE_DYNAMIC_IMPORT). Since the pages were code-split they land in
 * their own chunks, but those chunks are still preloaded with the entry.
 * Making them genuinely deferred means making the generated services import
 * lazily too, and `src/generated/` is regenerated by `pac` — not ours to
 * edit (see #7).
 */
